...
// MARK: - ILRadioPlayingState

/**
 `ILRadioPlayingState` is the Player playing state enum
 */

@objc public enum ILRadioPlaybackState: Int {
...
// MARK: - ILRadioPlayerState

/**
 `ILRadioPlayerState` is the Player status enum
 */

@objc public enum ILRadioPlayerState: Int {
...
@objc public protocol ILRadioPlayerDelegate: class {
    /**
     ...
     - parameter player: ILRadioPlayer
     - parameter state: ILRadioPlayerState
     */
    func radioPlayer(_ player: ILRadioPlayer, playerStateDidChange state: ILRadioPlayerState)
    
    /**
     Called when the player changes the playing state
     
     - parameter player: FRadioPlayer
     - parameter state: ILRadioPlaybackState
     */
    func radioPlayer(_ player: ILRadioPlayer, playbackStateDidChange state: ILRadioPlaybackState)
     ...
     */
    @objc optional func radioPlayer(_ player: ILRadioPlayer, itemDidChange url: URL?)
     ...
     */
    @objc optional func radioPlayer(_ player: ILRadioPlayer, metadataDidChange artistName: String?, trackName: String?)
     ...
     */
    @objc optional func radioPlayer(_ player: ILRadioPlayer, metadataDidChange rawValue: String?)
     ...
     */
    @objc optional func radioPlayer(_ player: ILRadioPlayer, artworkDidChange artworkURL: URL?)
}

// MARK: - ILRadioPlayer

/**
 ILRadioPlayer is a wrapper around AVPlayer to handle internet radio playback.
 */

open class ILRadioPlayer: NSObject {
    
    // MARK: - Properties
    
    /// Returns the singleton `FRadioPlayer` instance.
    public static let shared = ILRadioPlayer()
    
    /**
     The delegate object for the `ILRadioPlayer`.
     Implement the methods declared by the `ILRadioPlayerDelegate` object to respond to user interactions and the player output.
     */
    open weak var delegate: ILRadioPlayerDelegate?
    ...
    open private(set) var state = ILRadioPlayerState.urlNotSet {
        didSet {
    ...
    open private(set) var playbackState = ILRadioPlaybackState.stopped {
        didSet {
    ...
    private func shouldGetArtwork(for rawValue: String?, _ enabled: Bool) {
        ...
        ILRadioAPI.getArtwork(for: rawValue, size: artworkSize, completionHandler: { [unowned self] artworlURL in
            DispatchQueue.main.async {
